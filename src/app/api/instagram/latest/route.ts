...
export async function GET(request: NextRequest) {
  try {
    ...
    // Step 1: Get latest media from Instagram Graph API
    const mediaUrl = new URL(`https://graph.facebook.com/v23.0/${igUserId}/media`)
    mediaUrl.searchParams.set('fields', 'permalink,media_type,media_url,thumbnail_url,timestamp')
    ...
    const mediaResponse = await fetch(mediaUrl.toString())
         ...
         if (!mediaResponse.ok) {
       console.error('Instagram Graph API error:', mediaResponse.status, mediaResponse.statusText)
       return NextResponse.json(
         { 
           html: null, 
    ...
    const latestMedia = mediaData?.data?.[0] as InstagramMedia | undefined
    ...
    const oembedResponse = await fetch(oembedUrl.toString())
       ...
       console.error('Instagram oEmbed API error:', oembedResponse.status, oembedResponse.statusText)
       return NextResponse.json(
         { 
           html: null, 
