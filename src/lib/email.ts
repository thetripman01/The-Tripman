...
export async function sendCancellationNotification(booking: BookingWithEventType) {
  if (!resend) {
...
export async function sendRideStatusUpdate(
  ...
  status: string, 
  driverInfo?: { driverName?: string; driverPhone?: string; vehicleInfo?: string }
) {
  if (!resend) {
  ...
  const statusMessage = statusMessages[status as keyof typeof statusMessages] || 'Your ride status has been updated.'
