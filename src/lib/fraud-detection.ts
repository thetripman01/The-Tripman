...
export async function checkForFraud(bookingData: BookingData): Promise<FraudCheckResult> {
  ...
  try {
    ...
    if (duplicateBookings.length > 0) {
      riskScore += 30
      reasons.push(`Multiple bookings found for same email within 24 hours (${duplicateBookings.length} bookings)`)
      recommendations.push('Review booking history for this customer')
    }

    // 2. Check for suspicious email patterns
    const emailPatterns = [
      /^[a-z]+\d+@/, // starts with letters followed by numbers
      /test/i,
      /fake/i,
      /spam/i,
      /temp/i,
    ...
    const now = new Date()
    const hoursUntilBooking = (bookingData.startsAt.getTime() - now.getTime()) / (1000 * 60 * 60)
    
    if (hoursUntilBooking < 2) {
      riskScore += 25
      reasons.push('Booking made with less than 2 hours notice')
      recommendations.push('Require additional verification for short notice bookings')
    }

    // 4. Check for high-value bookings
    if (bookingData.amountPaid && bookingData.amountPaid > 10000) { // $100+
      riskScore += 15
      reasons.push('High-value booking detected')
      recommendations.push('Consider additional verification for high-value bookings')
    }
    ...
    if (recentBookings.length >= 3) {
      riskScore += 35
      reasons.push(`Multiple bookings created in short time (${recentBookings.length} in last hour)`)
      recommendations.push('Review booking patterns and consider rate limiting')
    }
    ...
    if (cancelledBookings.length >= 3) {
      riskScore += 25
      reasons.push(`Multiple cancelled bookings from same email (${cancelledBookings.length} in last 30 days)`)
    ...
    const isWeekend = bookingData.startsAt.getDay() === 0 || bookingData.startsAt.getDay() === 6
    ...
    if (refundedBookings.length >= 2) {
      riskScore += 20
      reasons.push(`Multiple refunded bookings (${refundedBookings.length} in last 90 days)`)
    ...
    if (riskScore >= 70) {
      recommendations.push('High risk - consider blocking or requiring additional verification')
    }
...
export function getFraudRiskLevel(riskScore: number): 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' {
